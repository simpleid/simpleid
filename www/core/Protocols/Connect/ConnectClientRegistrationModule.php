 ...
 * SimpleID
 *
 * Copyright (C) Kelvin Mo 2012-2026
 *
    ...
    public function register() {
        ...
        foreach ($request['redirect_uris'] as $redirect_uri) {
            ...
            if (!isset($parts['scheme'])) {
                $response->setError('invalid_redirect_uri', 'redirect_uris must contain a scheme')->renderJSON();
                return;
            }

            if (($application_type == 'web') && in_array('implicit', $grant_types)) {
                if ((strtolower($parts['scheme']) != 'https') || !isset($parts['host']) || ($parts['host'] == '127.0.0.1') || ($parts['host'] == '[::1]')) {
                ...
                if (((strtolower($parts['scheme']) == 'http') && ((($parts['host'] ?? '') != '127.0.0.1') && (($parts['host'] ?? '') != '[::1]')))
