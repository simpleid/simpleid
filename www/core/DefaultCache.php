...
/**
 * Default cache implementation using the filesytem.
 */
class DefaultCache implements Cache {
    ...
    function set($type, $key, $data, $time = NULL) {
        $filename = $this->getName($type, $key);
    ...
    function getAll($type) {
        ...
        $dir = opendir($this->cache_dir . '/' . $type);
        
        while (($file = readdir($dir)) !== false) {
            $filename = $this->cache_dir . '/' . $type . '/' . $file;
    ...
    protected function getName($type, $key) {
        return $this->cache_dir . '/' . $type . '/' . md5($key) . '.cache';
    }    
}
